...
- モデルと reasoning effort は `~/.codex/config.toml` の既定に依存せず明示指定する（既定は別用途＝コーディング委譲のために変わり得るため）。共通プレフィックスを `codex -m gpt-5.6-sol -c model_reasoning_effort="<effort>"` とし、`<effort>` には Step 3 で役割ごとに選定した値（`medium` または `xhigh`）を入れる。
