...
Claude Code 用スキル。claude（総合＝Fable 5.1、専門役割＝Opus 5.5 high）と codex（総合＝gpt-6-sol）に並列でコードレビューを依頼し、呼び出し元が指摘を選別・修正指揮し、未解決の妥当な指摘がなくなるまで再レビューを繰り返す。codex の総合レビュアーの reasoning effort は medium 固定。
