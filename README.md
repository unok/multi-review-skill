...
Claude Code 用スキル。claude（汎用・Fable 5.1）と codex（役割別）に並列でコードレビューを依頼し、呼び出し元が指摘を選別・修正指揮し、未解決の妥当な指摘がなくなるまで再レビューを繰り返す。codex 役割レビュアーは役割ごとに reasoning effort を選定する（既定 medium、高難度の役割のみ xhigh）。
