...
Claude Code 用スキル。claude（総合・Fable 5.1）と codex（総合＝gpt-6-astra、専門役割＝gpt-5.6-sol）に並列でコードレビューを依頼し、呼び出し元が指摘を選別・修正指揮し、未解決の妥当な指摘がなくなるまで再レビューを繰り返す。codex 役割レビュアーは役割ごとに reasoning effort を選定する（既定 high、高難度の専門役割のみ xhigh）。
...
- `references/roles.md` — 役割カタログ（総合 / セキュリティ / バグハント / アーキテクチャなど11役割）
- `CONTEXT.md` — 用語集
